/* 
        folab-handout.zipr (int j = 0; j < SIZE; j++) {
 * CS:APP Data Lab 
 * 
...
int logicalShift(int x, int n) {
	...
	//make an f-ing wide 0x11... and shift/flip it to make the correct mask
...
int allOddBits(int x) {
	...
	//making a giant mask like a dumba$$ just mask it like a pro
...
int conditional(int x, int y, int z) {
    	int returnval = 0;
	x = !!x;
	//standardize to 0x1 or 0x0
	x = x<<31;
	x = x>>31;
	//standardize to 0xFF... or 0x00...
	returnval = (y & x) | (z & ~x);
	//combine with flipped/unflipped versions
...
int logicalNeg(int x) {
	...
	int xtended = x>>31;
	int flipxtended = (~x + 1)>>31 ; //negative x (+1 for 2's comp offset)
	returnval = (xtended | flipxtended) + 1;
